...
def det_yolov5v6(info1,info2,info3):
    result = []
                    ...
                    for *xyxy, conf, cls in reversed(det):
                        ...
                        if save_img or save_crop or view_img:  # Add bbox to image
                            ...
                            try:
                                ...
                                imgcut = img_input[int(c1[1])+4:int(c2[1]-2), int(c1[0])+4:int(c2[0])-2]
                                imgcut = cv2.resize(imgcut, (0, 0), fx=2, fy=2, interpolation=cv2.INTER_NEAREST)
                                num += 1
                                Img_name = str(num) + ".jpg"
                                img_file = info3  # "d:/Math/Temporary_folder"
                                if not os.path.exists(img_file):
                                    os.mkdir(img_file)
                                num2 += 1
                                if num2 == 30:
                                    num2 = 0
                                    #cv2.imwrite(img_file + "/" + Img_name, imgcut, [int(cv2.IMWRITE_JPEG_QUALITY), 100])
    ...
    def parse_opt():
        ...
        parser.add_argument('--weights', nargs='+', type=str, default=info1, help='model path(s)')
        parser.add_argument('--source', type=str, default=info2, help='file/dir/URL/glob, 0 for webcam')
